...
namespace Beacon.API.Commands
{
    public abstract class BeaconCommand 
    {
        ...
        public BeaconCommand? GetSubCommand(string keyword) => _subcommands.GetValueOrDefault(keyword);
        ...
        /// </summary>
        /// <param name="keyword">The keyword for the subcommand.</param>
        /// <param name="command"></param> A function to be called once the sub-commands gets called.
        /// <returns>The sub-command that has been added.</returns>
        protected BeaconCommand AddSubCommand(string keyword, Func<ICommandSender, string[], CancellationToken, ValueTask<bool>> action)
        {
